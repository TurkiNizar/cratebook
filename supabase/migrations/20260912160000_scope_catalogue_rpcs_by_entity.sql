...
declare
  ...
  v_external_entity_type public.catalogue_entity_type := coalesce(
    nullif(p_source_data ->> 'entityType', '')::public.catalogue_entity_type,
...
begin
  ...
  perform pg_advisory_xact_lock(
    hashtextextended(
      v_user_id::text || ':' || p_external_source || ':'
        || v_external_entity_type::text || ':' || p_external_id,
      0
    )
  );
  ...
  where created_by = v_user_id
    ...
    and external_entity_type = v_external_entity_type
...
comment on function public.create_catalogue_collection_item is
  'Atomically creates a private copy from verified, entity-scoped catalogue metadata, reusing a matching owner release and backfilling only missing artwork.';
...
comment on function public.create_catalogue_wishlist_item is
  'Atomically creates a wishlist item from verified, entity-scoped catalogue metadata, reusing a matching owner release and backfilling only missing artwork.';
