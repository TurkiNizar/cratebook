alter table public.releases
  add column artwork_data jsonb;

alter table public.releases
  add constraint releases_artwork_provenance_valid check (
    artwork_data is null
    or (
      jsonb_typeof(artwork_data) = 'object'
      and artwork_data ->> 'provider' = 'musicbrainz'
      and artwork_data ->> 'entityType' = 'release_group'
      and artwork_data #>> '{releaseGroup,id}'
        ~* '^[0-9a-f]{8}-[0-9a-f]{4}-[1-8][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$'
      and cover_url = artwork_data #>> '{coverArt,thumbnailUrl}'
      and (artwork_data #>> '{coverArt,thumbnailUrl}')
        ~* '^https://(www\.)?coverartarchive\.org/(release|release-group)/[0-9a-f-]+/[^[:space:]]+$'
      and (artwork_data #>> '{coverArt,originalUrl}')
        ~* '^https://(www\.)?coverartarchive\.org/(release|release-group)/[0-9a-f-]+/[^[:space:]]+$'
      and octet_length(artwork_data::text) <= 8192
    )
  );

comment on column public.releases.artwork_data is
  'Bounded private provenance for independently selected representative artwork. It does not change or replace the release catalogue identity.';
...
comment on function public.update_collection_item_details is
  'Atomically updates an authenticated user''s release, physical-copy details, normalized tags, and optionally its independently attributed artwork.';
